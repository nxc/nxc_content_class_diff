	...
	public static function parseClassesDefinition( DOMDocument $dom ) {
		...
		foreach( $classNodes as $classNode ) {
			...
			$class = array(
				...
				'object_name_patter' => is_object( $classNode->getElementsByTagName( 'object-name-pattern' )->item( 0 ) )
					? $classNode->getElementsByTagName( 'object-name-pattern' )->item( 0 )->textContent
					: null,
				'is_container'       => $classNode->getAttribute( 'is-container' ) == 'true',
				'attributes'         => array()
			);
			...
			foreach( $classAttributeList as $classAttributeNode ) {
				...
				if( $datatypeParameterNodes->length > 0 ) {
					foreach( $datatypeParameterNodes as $datatypeParameterNode ) {
						if( $datatypeParameterNode instanceof DOMText ) {
							...
							$datatypeParameters[ $datatypeParameterNode->tagName ] = trim( $datatypeParameterNode->textContent );
						}
					}
				}
				...
				$attribute = array(
					...
					'required'              => strtolower( $classAttributeNode->getAttribute( 'required' ) ) == 'true',
					'searchable'            => strtolower( $classAttributeNode->getAttribute( 'searchable' ) ) == 'true',
					'information_collector' => strtolower( $classAttributeNode->getAttribute( 'information-collector' ) ) == 'true',
